...
const TutorMetric = () => {
  ...
  const tutorSessions = useSelector(state => state.sessions.allSessionsByTutor)
    ...
    const clientSessions = useSelector(
      state => state.sessions.allSessionsByClient
    )
    const clientSessionsData = useSelector(
      state => state.sessions.allSessionsDataByClient
    )
...
const currentMonthPrice = currentMonthData.reduce((total, session) => total + session.price, 0);
const previousMonthPrice = previousMonthData.reduce((total, session) => total + session.price, 0);
...
const delta = ((currentMonthPrice - previousMonthPrice) / previousMonthPrice) * 100;
const deltaType = delta > 0 ? "moderateIncrease" : "moderateDecrease";

const currentMonthProfit = currentMonthPrice * 0.90;
const previousMonthProfit = previousMonthPrice * 0.90;
...
const deltaProfit = ((currentMonthProfit - previousMonthProfit) / previousMonthProfit) * 100;
const deltaSessions = ((currentMonthSessions - previousMonthSessions) / previousMonthSessions) * 100;
const deltaSessionsFormatted = `${deltaSessions.toFixed(1)}%`;

const deltaTypePrice = delta > 0 ? "moderateIncrease" : "moderateDecrease";
const deltaTypeProfit = deltaProfit > 0 ? "moderateIncrease" : "moderateDecrease";
const deltaTypeSessions = deltaSessions > 0 ? "moderateIncrease" : "moderateDecrease";
  ...
  const categories = [
    {
      ...
      delta: previousMonthPrice !== 0 ? `${delta.toFixed(1)}%` : "No posees datos",
    ...
    },
    {
      ...
      delta: previousMonthProfit !== 0 ? `${deltaProfit.toFixed(1)}%` : "No posees datos",
    ...
    },
    {
      ...
      delta: previousMonthSessions !== 0 ? deltaSessionsFormatted : "No posees datos",
          ...
          <Card key={item.title}>
            ...
            <Flex justifyContent="start" className="space-x-2 mt-4">
              ...
              <Flex justifyContent="start" className="space-x-1 truncate">
                <Text className=" text-xs" color={colors[calculateDeltaType(item)]}>{item.delta}</Text>
